...
(use-package! agent-shell
  ...
  ;; autoloads de `agent-shell-hermes.el', que podem ficar desatualizados após
  ;; uma atualização do agent-shell no Doom.
  :demand t
  :config
  (require 'acp)
  (require 'agent-shell-hermes)

  ;; O cliente ACP usa o SSH somente como transporte de stdin/stdout. Não crie
  ;; pseudo-terminal: qualquer saída de shell misturada ao stdout quebra o
  ;; protocolo JSON-RPC do ACP.
  (setq agent-shell-hermes-acp-command
        ...
        agent-shell-hermes-environment
        (agent-shell-make-environment-variables :inherit-env t)
        ;; Aprova edições no workspace; caminhos sensíveis continuam pedindo
        ;; confirmação.
        agent-shell-hermes-default-session-mode-id "accept_edits"
        ;; Usa Hermes diretamente, sem abrir o seletor de agentes.
